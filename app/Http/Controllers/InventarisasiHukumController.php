...
class InventarisasiHukumController extends Controller
{
    ...
    public function abstrak($id, $format)
    {
        ...
        $pdf::SetTitle($data[0]->link);
        $pdf::setImageScale(PDF_IMAGE_SCALE_RATIO);
        $pdf::SetDisplayMode('fullpage', 'OneColumn', 'UseThumbs');
        ...
        ob_end_clean();
        $pdf::Output($data[0]->link . '.pdf', 'I');
    }

    public function kategori($param)
    {
        $data = InventarisasiHukum::select(['kategori.nama', 'inventarisasi_hukum.*', 'abstrak.*', 'inventarisasi_hukum.id as idph'])
