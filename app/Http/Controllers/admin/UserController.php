    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                ...
                'role' => 'required|exists:users_role,id'
            ], [
                'password.min' => 'Password must be at least 8 characters.',
                'password.regex' => 'Password must include an uppercase, lowercase, number, and special character.',
            ]);
            ...
            return response()->json([
                ...
                'errors' => $e->errors() // detailed validation messages
            ], 422);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
                'message' => 'Tambah user baru gagal',
                'debug' => $e->getMessage()
            ], 400);
        }
    }
